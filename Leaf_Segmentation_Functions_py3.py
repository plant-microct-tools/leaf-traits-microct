...
def Threshold_GridPhase_invert_down(grid_img, phase_img, Th_grid, Th_phase, folder_name, sample_name, rescale_factor, threshold_rescale_factor, phase_only=False):
...
def Load_Resize_and_Save_Stack(filepath, stack_name, rescale_factor, threshold_rescale_factor,
    ...
    if os.path.isfile(filepath + stack_name + "_" + str(rescale_factor) + "x-smaller.tif"):
        ...
        if (rescale_factor == 1) & (threshold_rescale_factor == 1):
            ...
            stack_rs = np.empty(shape = resized_shape.astype(np.int64), dtype=np.int64)
